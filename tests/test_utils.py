...
class TestShapeDetector(ShapeSetUp):
    ...
    def test_large_circle(self):
        arr = pydicom.dcmread(self.LARGE_CIRCLE_PHANTOM_FILE).pixel_array
    ...
    def test_small_circle(self):
        arr = pydicom.dcmread(self.SMALL_CIRCLE_PHANTOM_FILE).pixel_array
        shape_detector = hazen_tools.ShapeDetector(arr=arr)
        x, y, r = shape_detector.get_shape("circle")
    ...
    def test_sag_rectangle(self):
        arr = pydicom.dcmread(self.SAG_RECTANGLE_PHANTOM_FILE).pixel_array
        shape_detector = hazen_tools.ShapeDetector(arr=arr)
    ...
    def test_cor_rectangle(self):
        arr = pydicom.dcmread(self.COR_RECTANGLE_PHANTOM_FILE).pixel_array
        shape_detector = hazen_tools.ShapeDetector(arr=arr)
        centre, size, angle = shape_detector.get_shape("rectangle")
    ...
    def test_cor2_rectangle(self):
        arr = pydicom.dcmread(self.COR2_RECTANGLE_PHANTOM_FILE).pixel_array
        shape_detector = hazen_tools.ShapeDetector(arr=arr)
        centre, size, angle = shape_detector.get_shape("rectangle")
...
class TestUtils(unittest.TestCase):
    def setUp(self):
        ...
        TEST_DICOM = pydicom.dcmread(TEST_DICOM)
    ...
    def test_is_enhanced(self):
        for manufacturer in self.test_dicoms.keys():
            with pydicom.dcmread(self.test_dicoms[manufacturer]["file"]) as dcm:
